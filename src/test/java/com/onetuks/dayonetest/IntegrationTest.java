...
@ContextConfiguration(
    initializers =
        IntegrationTest.IntegrationTestIntializer
            .class) // 이 어노테이션으로 ApplicatinoContext 초기화 시 커스텀한 설정으로 생성된다
public class IntegrationTest {
  ...
  // ApplicationContext 가 처음 초기화될 때 TestContainers 속성이 아니라, application.properties 설정값을 사용하기 때문에 DB
  // 연결 안 되는 문제 해결하려는 목적
  // ApplicationContext 생성 시 TestContainers 를 통해 만든 rdbms 호스트, 포트를 사용하여 테스트용 DB에 연결한다.
  static class IntegrationTestIntializer
